...
export default function ContentCard({
    ...
    const [canSeeVideo, setCanSeeVideo] = useState(false)

    const getBuysFromAuthor = async () => {
        const buys = await coursesContract?.methods?.getAllBuys().call()


        const myBuys = buys.filter(buy => buy.from.toLowerCase() === wallet.toLowerCase() )
        if (myBuys.length > 0) {
            const hasBuyTheContent = myBuys.filter(buy => buy.idContent.toString() === id.toString())
            setCanSeeVideo(hasBuyTheContent.length > 0)
        } else {
            setCanSeeVideo(false)
        }
        console.log('buys', myBuys)

    }

    const checkIfUserHasBuyTheContent = () => {
    ...
    return (
        <Container maxW={'1200'} p={14}>
            <Card p={10} m={20}>
                ...
                <HStack>
                    {video && (canSeeVideo || author.toLowerCase() === wallet.toLowerCase())?
